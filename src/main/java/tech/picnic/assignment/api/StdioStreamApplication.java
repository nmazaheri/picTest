...
public final class StdioStreamApplication {
    ...
    public static void main(String[] args) throws IOException {
        if (args.length != 2) {
            System.err.printf("Usage:%n  java -jar application.jar <maxEvents> <maxReadTime>%n");
            System.exit(1);
        }

        int maxEvents = Integer.parseInt(args[0]);
        Duration maxReadTime = Duration.parse(args[1]);

        processStdio(maxEvents, maxReadTime);
    }

    private static void processStdio(int maxEvents, Duration maxReadTime) throws IOException {
